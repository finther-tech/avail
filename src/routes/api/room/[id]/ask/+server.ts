...
export const POST: RequestHandler = async ({ request, params }) => {
	...
	if (!question || typeof question !== 'string') {
		return json({ answer: 'Please ask a question.' });
	}

	// Verify room exists
	const room = await getRoom(roomId);
	if (!room) {
		return json({ answer: 'Room not found.' }, { status: 404 });
	}
	...
	try {
		...
		return json({
			answer: `Error: ${errorMessage}. Please try again or [book manually](/room/${roomId}/book).`
		});
	}
};
